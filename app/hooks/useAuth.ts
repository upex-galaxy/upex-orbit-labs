...
export const useAuth = () => {
  ...
  const validateCredentials = (data: LoginFormData): LoginError | null => {
    ...
    if (!username && !password) return { message: AUTH_MESSAGES.MISSING_CREDENTIALS };
  ...
  const login = (data: LoginFormData) => {
    ...
    const customMessage = AUTH_MESSAGES.CUSTOM_MESSAGES[lowerCaseUsername as keyof typeof AUTH_MESSAGES.CUSTOM_MESSAGES];
    if (customMessage) {
